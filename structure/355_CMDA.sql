...
CREATE OR REPLACE DYNAMIC TABLE CMDA_AGG_DT_TRADE_SUMMARY(
    ...
    SETTLEMENT_DATE COMMENT 'Settlement/delivery date',
    CUSTOMER_ID COMMENT 'Customer identifier',
    ACCOUNT_ID COMMENT 'Investment account',
    COMMODITY_TYPE COMMENT 'ENERGY, PRECIOUS_METAL, BASE_METAL, AGRICULTURAL',
    COMMODITY_NAME COMMENT 'Specific commodity name',
    COMMODITY_CODE COMMENT 'Commodity code',
    CONTRACT_TYPE COMMENT 'SPOT, FUTURE, FORWARD, SWAP',
    SIDE COMMENT '1=Buy, 2=Sell',
    SIDE_DESCRIPTION COMMENT 'BUY or SELL',
    QUANTITY COMMENT 'Quantity in commodity units',
    UNIT COMMENT 'Unit of measure',
    PRICE COMMENT 'Price per unit',
    CURRENCY COMMENT 'Trading currency',
    GROSS_AMOUNT COMMENT 'Gross amount in trade currency',
    ...
    FX_RATE COMMENT 'Exchange rate to CHF',
    DELTA COMMENT 'Price sensitivity in CHF',
    SPOT_PRICE COMMENT 'Current spot price',
    FORWARD_PRICE COMMENT 'Forward/futures price',
    VOLATILITY COMMENT 'Price volatility (%)',
    LIQUIDITY_SCORE COMMENT 'Liquidity score (1-10)',
    EXCHANGE COMMENT 'Trading exchange',
    DELIVERY_MONTH COMMENT 'Delivery month',
    DELIVERY_LOCATION COMMENT 'Delivery location',
    TRADE_VALUE_CATEGORY COMMENT 'SMALL/MEDIUM/LARGE/VERY_LARGE',
    VOLATILITY_REGIME COMMENT 'LOW/NORMAL/HIGH/EXTREME',
...
CREATE OR REPLACE DYNAMIC TABLE CMDA_AGG_DT_PORTFOLIO_POSITIONS(
    ...
    COMMODITY_CODE COMMENT 'Commodity code',
    UNIT COMMENT 'Unit of measure',
    TOTAL_QUANTITY COMMENT 'Net position quantity',
    TOTAL_BUY_QUANTITY COMMENT 'Total quantity bought',
    TOTAL_SELL_QUANTITY COMMENT 'Total quantity sold',
    TOTAL_TRADES COMMENT 'Number of trades',
    TOTAL_INVESTMENT_CHF COMMENT 'Total investment in CHF',
    NET_INVESTMENT_CHF COMMENT 'Net investment (buys - sells) in CHF',
    REALIZED_PL_CHF COMMENT 'Realized profit/loss in CHF',
    AVERAGE_PRICE COMMENT 'Volume-weighted average price',
    TOTAL_DELTA_CHF COMMENT 'Total delta in CHF',
    AVERAGE_VOLATILITY COMMENT 'Average volatility',
    POSITION_STATUS COMMENT 'LONG/SHORT/CLOSED',
...
CREATE OR REPLACE DYNAMIC TABLE CMDA_AGG_DT_DELTA_EXPOSURE(
    ...
    TOTAL_DELTA_CHF COMMENT 'Total delta in CHF',
    LONG_DELTA_CHF COMMENT 'Long delta in CHF',
    SHORT_DELTA_CHF COMMENT 'Short delta in CHF',
    NET_DELTA_CHF COMMENT 'Net delta exposure in CHF',
    LARGEST_SINGLE_POSITION_CHF COMMENT 'Largest single position value',
...
CREATE OR REPLACE DYNAMIC TABLE CMDA_AGG_DT_VOLATILITY_ANALYSIS(
    COMMODITY_NAME COMMENT 'Specific commodity name',
    COMMODITY_TYPE COMMENT 'ENERGY, PRECIOUS_METAL, BASE_METAL, AGRICULTURAL',
    CURRENT_VOLATILITY COMMENT 'Current volatility (%)',
    MIN_VOLATILITY COMMENT 'Minimum observed volatility',
    MAX_VOLATILITY COMMENT 'Maximum observed volatility',
    VOLATILITY_REGIME COMMENT 'LOW/NORMAL/HIGH/EXTREME',
    TRADE_COUNT COMMENT 'Number of trades',
    TOTAL_VALUE_CHF COMMENT 'Total trade value in CHF',
    LAST_UPDATED COMMENT 'Timestamp when calculated'
...
CREATE OR REPLACE DYNAMIC TABLE CMDA_AGG_DT_DELIVERY_SCHEDULE(
    DELIVERY_MONTH COMMENT 'Delivery month (YYYY-MM)',
    DELIVERY_LOCATION COMMENT 'Delivery location/hub',
    COMMODITY_TYPE COMMENT 'ENERGY, PRECIOUS_METAL, BASE_METAL, AGRICULTURAL',
    COMMODITY_NAME COMMENT 'Specific commodity name',
    TOTAL_QUANTITY_TO_DELIVER COMMENT 'Total quantity for delivery (sells)',
    TOTAL_QUANTITY_TO_RECEIVE COMMENT 'Total quantity to receive (buys)',
    NET_DELIVERY_OBLIGATION COMMENT 'Net delivery obligation',
    UNIT COMMENT 'Unit of measure',
    CONTRACT_COUNT COMMENT 'Number of contracts',
    EARLIEST_TRADE_DATE COMMENT 'Earliest trade date',
    LAST_UPDATED COMMENT 'Timestamp when calculated'
