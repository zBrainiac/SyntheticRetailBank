...
CREATE OR REPLACE DYNAMIC TABLE EQTA_AGG_DT_TRADE_SUMMARY(
    TRADE_ID COMMENT 'Unique trade identifier',
    TRADE_DATE COMMENT 'Trade execution timestamp',
    SETTLEMENT_DATE COMMENT 'Settlement date for cash/securities transfer',
    CUSTOMER_ID COMMENT 'Customer who executed the trade',
    ACCOUNT_ID COMMENT 'Investment account used for settlement',
    ORDER_ID COMMENT 'Order reference for trade grouping',
    EXEC_ID COMMENT 'Execution reference',
    SYMBOL COMMENT 'Stock symbol/ticker',
    ISIN COMMENT 'International Securities Identification Number',
    SIDE COMMENT 'Trade side (1=Buy, 2=Sell)',
    SIDE_DESCRIPTION COMMENT 'Trade side description (BUY/SELL)',
    QUANTITY COMMENT 'Number of shares/units traded',
    PRICE COMMENT 'Execution price per share',
    CURRENCY COMMENT 'Trade currency',
    GROSS_AMOUNT COMMENT 'Gross trade amount in trade currency',
    COMMISSION COMMENT 'Trading commission charged',
    NET_AMOUNT COMMENT 'Net amount after commission',
    BASE_CURRENCY COMMENT 'Base reporting currency (CHF)',
    BASE_GROSS_AMOUNT COMMENT 'Gross amount in CHF',
    BASE_NET_AMOUNT COMMENT 'Net amount in CHF',
    FX_RATE COMMENT 'Exchange rate used for conversion to CHF',
    MARKET COMMENT 'Exchange/market where trade was executed',
    ORDER_TYPE COMMENT 'Order type (MARKET/LIMIT/STOP)',
    EXEC_TYPE COMMENT 'Execution type (NEW/FILL/PARTIAL_FILL)',
    TIME_IN_FORCE COMMENT 'Time in force instruction (DAY/GTC/IOC)',
    BROKER_ID COMMENT 'Executing broker identifier',
    VENUE COMMENT 'Trading venue',
    COMMISSION_RATE_BPS COMMENT 'Commission rate in basis points (bps)',
    TRADE_VALUE_CATEGORY COMMENT 'Trade size category (SMALL/MEDIUM/LARGE/VERY_LARGE)',
    SETTLEMENT_DAYS COMMENT 'Days between trade and settlement',
    TRADE_YEAR COMMENT 'Year of trade execution',
    TRADE_MONTH COMMENT 'Month of trade execution',
    TRADE_DAY_OF_WEEK COMMENT 'Day of week (1=Monday, 7=Sunday)',
    CREATED_AT COMMENT 'Timestamp when trade was recorded in system'
...
CREATE OR REPLACE DYNAMIC TABLE EQTA_AGG_DT_PORTFOLIO_POSITIONS(
    ...
    SYMBOL COMMENT 'Stock symbol/ticker',
    ISIN COMMENT 'International Securities Identification Number',
    CURRENCY COMMENT 'Trading currency for this position',
    TOTAL_QUANTITY COMMENT 'Net position quantity (positive=long, negative=short, zero=closed)',
    TOTAL_BUYS COMMENT 'Total shares purchased',
    TOTAL_SELLS COMMENT 'Total shares sold',
    AVERAGE_BUY_PRICE COMMENT 'Volume-weighted average buy price',
    AVERAGE_SELL_PRICE COMMENT 'Volume-weighted average sell price',
    TOTAL_BUY_AMOUNT COMMENT 'Total amount spent on purchases (in trade currency)',
    TOTAL_SELL_AMOUNT COMMENT 'Total amount received from sales (in trade currency)',
    TOTAL_COMMISSION COMMENT 'Total commission paid on all trades',
    NET_INVESTMENT COMMENT 'Net investment amount (buys - sells + commission)',
    TOTAL_BUY_AMOUNT_CHF COMMENT 'Total purchase amount in CHF',
    TOTAL_SELL_AMOUNT_CHF COMMENT 'Total sales amount in CHF',
    NET_INVESTMENT_CHF COMMENT 'Net investment in CHF',
    REALIZED_PL_CHF COMMENT 'Realized profit/loss in CHF (for closed/partial positions)',
    POSITION_STATUS COMMENT 'Position status (LONG/SHORT/CLOSED)',
    FIRST_TRADE_DATE COMMENT 'Date of first trade for this position',
    LAST_TRADE_DATE COMMENT 'Date of most recent trade',
...
CREATE OR REPLACE DYNAMIC TABLE EQTA_AGG_DT_CUSTOMER_ACTIVITY(
    CUSTOMER_ID COMMENT 'Customer identifier',
    TOTAL_TRADES COMMENT 'Total number of trades executed',
    TOTAL_BUY_TRADES COMMENT 'Number of buy trades',
    TOTAL_SELL_TRADES COMMENT 'Number of sell trades',
    UNIQUE_SYMBOLS COMMENT 'Number of unique symbols traded',
    UNIQUE_ACCOUNTS COMMENT 'Number of investment accounts used',
    TOTAL_VOLUME_CHF COMMENT 'Total trading volume in CHF (sum of all trade values)',
    TOTAL_COMMISSION_CHF COMMENT 'Total commission paid in CHF',
    AVERAGE_TRADE_SIZE_CHF COMMENT 'Average trade size in CHF',
    LARGEST_TRADE_CHF COMMENT 'Largest single trade value in CHF',
    AVERAGE_COMMISSION_BPS COMMENT 'Average commission rate in basis points',
    FIRST_TRADE_DATE COMMENT 'Date of first trade',
    LAST_TRADE_DATE COMMENT 'Date of most recent trade',
    TRADING_DAYS COMMENT 'Number of distinct days with trading activity',
    CUSTOMER_TENURE_DAYS COMMENT 'Days since first trade',
    AVERAGE_TRADES_PER_MONTH COMMENT 'Average number of trades per month',
    MOST_TRADED_SYMBOL COMMENT 'Symbol with highest trade count',
    MOST_TRADED_SYMBOL_COUNT COMMENT 'Number of trades for most traded symbol',
    PREFERRED_MARKET COMMENT 'Market with highest trade count',
    PREFERRED_ORDER_TYPE COMMENT 'Most frequently used order type',
    TRADER_CATEGORY COMMENT 'Trading activity category (VERY_ACTIVE/ACTIVE/MODERATE/OCCASIONAL/INACTIVE)',
    LAST_UPDATED COMMENT 'Timestamp when metrics were calculated'
