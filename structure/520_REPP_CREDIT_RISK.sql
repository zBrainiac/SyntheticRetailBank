...
CREATE OR REPLACE DYNAMIC TABLE REPP_AGG_DT_CUSTOMER_RATING_HISTORY(
    CUSTOMER_ID COMMENT 'Customer identifier for historical tracking',
    EFFECTIVE_DATE COMMENT 'Date when this rating became effective',
    CREDIT_RATING COMMENT 'Credit rating at this point in time',
    PD_1_YEAR COMMENT 'Probability of Default (1-year) at this point in time',
    PD_LIFETIME COMMENT 'Lifetime Probability of Default at this point in time',
    LGD_RATE COMMENT 'Loss Given Default rate at this point in time',
    RISK_WEIGHT COMMENT 'Risk weight percentage at this point in time',
    DEFAULT_FLAG COMMENT 'Whether customer was in default at this point in time',
    WATCH_LIST_FLAG COMMENT 'Whether customer was on watch list at this point in time',
    TOTAL_EXPOSURE_CHF COMMENT 'Total exposure amount at this point in time',
    DAYS_PAST_DUE COMMENT 'Days past due at this point in time',
    RATING_DATE COMMENT 'Date when rating was calculated'
